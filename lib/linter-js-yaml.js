...
import { localTags, build as buildCfTags, genSchema } from 'cloudformation-js-yaml-schema';

export default {
  Schema: genSchema(yaml),
  config: {
    customTags: {
      type: 'array',
      default: [],
  ...
  provideLinter() {
    return {
      ...
      lint: (TextEditor) => {
        ...
        try {
          // this.Schema = this.Schema ? this.Schema : genSchema(yaml);
          // console.log(require('util').inspect(this.Schema, { depth: 4 }));
